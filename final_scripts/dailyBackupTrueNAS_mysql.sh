...
databases=`sudo mysql -u $USER -p$PASSWORD -e "SHOW DATABASES;" | tr -d "| " | grep -v -e "Database" -e "information_schema" -e "mysql" -e "performance_schema"`
...
for i in $databases; do
	...
	sudo mysqldump -u $USER -p$PASSWORD -q --databases $i | gzip > $backupfile  2>"$temp_err_file"
    dump_exit_code=$?
    e_time=$(date +%s)
