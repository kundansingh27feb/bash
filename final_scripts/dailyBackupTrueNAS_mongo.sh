...
USER="mongo_admin"
PASSWORD="P66vXxhUJi2bSZ9"
AUTHENTICATION_DB="graylog"
...
databases=$(mongosh --username $USER --password $PASSWORD --authenticationDatabase $AUTHENTICATION_DB --quiet --eval "db.adminCommand('listDatabases').databases.forEach(db => print(db.name))" | grep -w -v -e "admin" -e "config" -e "local" | sed -e 's/ //g' -e '/^$/d')
echo "$(date) Starting backup of databases $backup_date " >"$logfile"
for i in $databases; do
    ls_time=$(date +"%T")
    backupfile="$local_backup_dir/$i.$backup_date.dump"
    
    echo Dumping $i to $backupfile
    s_time=$(date +%s)
    temp_err_file=$(mktemp)
	mongodump --username $USER --password $PASSWORD --authenticationDatabase $AUTHENTICATION_DB --db $i --gzip --archive=$backupfile 1>>"$logfile" 2>"$temp_err_file"
    dump_exit_code=$?
    e_time=$(date +%s)
    ...
    if [ "$dump_exit_code" -eq 0 ]; then
        echo "<tr align=center><td>$i</td><td>$t_taken</td><td>$dumpsize</td><td>None</td></tr>" >>"$bckreport"
