...
<tr align='center'><td colspan='4'><b>$dbtype Backup Details</b></td></tr>
...
for i in $databases; do
	if echo "$i" | tr '[:upper:]' '[:lower:]' | grep -q "zip"; then
		ls_time=$(date +"%T")
		backupfile="$local_backup_dir/$i.zipmdm_table.$backup_date.sql.gz"
		echo Dumping $i zipmdm_table to $backupfile
		s_time=$(date +%s)
		temp_err_file=$(mktemp)
		sudo -u postgres pg_dump -Z1 -Fc -d "$i" -t public.* >"$backupfile" 2>"$temp_err_file"
		dump_exit_code=$?
		e_time=$(date +%s)
		echo "$(date) Backup and Vacuum complete on $backup_date for database: $i.zipmdm_table " >>"$logfile"
		...
		if [ "$dump_exit_code" -eq 0 ]; then
			echo "<tr align=center><td>$i.zipmdm_table</td><td>$t_taken</td><td>$dumpsize</td><td>None</td></tr>" >>"$bckreport"
		elif [ "$dump_exit_code" -eq 1 ]; then
			warning_count=$((warning_count + 1))
			warning_messages+="Warning for $i: $(cat "$temp_err_file")"$'\n'
			echo "<tr align=center><td>$i.zipmdm_table</td><td>$t_taken</td><td>$dumpsize</td><td>Warning Count: $warning_count, Warning: $warning_messages</td></tr>" >>"$bckreport"
		else
			error_count=$((error_count + 1))
			error_messages+="Error for $i: $(cat "$temp_err_file")"$'\n'
			echo "<tr align=center><td>$i.zipmdm_table</td><td>$t_taken</td><td>$dumpsize</td><td>Error Count: $error_count, Error: $error_messages</td></tr>" >>"$bckreport"
		fi
		le_time=$(date +"%T")
  ...
  \"Local_Backup_Schema_Name\": \"$i.zipmdm_table\",
  ...
  \"Local_Backup_Time_Taken\": \"$t_taken\"
		...
		ls_time=$(date +"%T")
		backupfile="$local_backup_dir/$i.largeobject_metadata_tables.$backup_date.sql.gz"
		echo Dumping $i objectfile to $backupfile
		s_time=$(date +%s)
		temp_err_file=$(mktemp)
		sudo -u postgres pg_dump -Z1 -Fc "$i" -t pg_largeobject -t pg_largeobject_metadata >"$backupfile" 2>"$temp_err_file"
		dump_exit_code=$?
		e_time=$(date +%s)
		echo "$(date) Backup and Vacuum complete on $backup_date for database: $i.objectfile " >>"$logfile"
		t_taken=$(echo $((e_time - s_time)) | awk '{printf "%d:%02d:%02d", $1 / 3600, ($1 / 60) % 60, $1 % 60}')
		dumpsize=$(ls -ltr --block-size=M "$backupfile" | awk -F" " '{print $5}')
		dumpsize_json=$(du -b $backupfile | awk -F" " '{print $1}')
		if [ "$dump_exit_code" -eq 0 ]; then
			echo "<tr align=center><td>$i.objectfile</td><td>$t_taken</td><td>$dumpsize</td><td>None</td></tr>" >>"$bckreport"
		elif [ "$dump_exit_code" -eq 1 ]; then
			warning_count=$((warning_count + 1))
			warning_messages+="Warning for $i: $(cat "$temp_err_file")"$'\n'
			echo "<tr align=center><td>$i.objectfile</td><td>$t_taken</td><td>$dumpsize</td><td>Warning Count: $warning_count, Warning: $warning_messages</td></tr>" >>"$bckreport"
		else
			error_count=$((error_count + 1))
			error_messages+="Error for $i: $(cat "$temp_err_file")"$'\n'
			echo "<tr align=center><td>$i.objectfile</td><td>$t_taken</td><td>$dumpsize</td><td>Error Count: $error_count, Error: $error_messages</td></tr>" >>"$bckreport"
		fi
		le_time=$(date +"%T")
  ...
  \"Local_DB_Name\": \"$host-$ipaddr\",
  \"Local_Backup_Start_Time\": \"$ls_time\",
  \"Local_Backup_Schema_Name\": \"$i.objectfile\",
  \"Local_Backup_Schema_Size\": \"$dumpsize_json\",
  \"Local_Backup_End_Time\": \"$le_time\",
  \"Local_Backup_Date\": \"$backup_date\",
  \"Local_Backup_Time_Taken\": \"$t_taken\"
...
if [ $? -eq 0 ]; then
    ...
    if [ "$day_of_week" -eq 1 ]; then
		...
		last_day_size_bytes=$(du -b "$weekly$last_backup_date" | awk -F" " '{print $1}')
    else
        last_day_size=$(du -sh "$backup_dir$last_backup_date" | awk -F" " '{print $1}')
		last_day_size_bytes=$(du -b "$backup_dir$last_backup_date" | awk -F" " '{print $1}')
    fi
    ...
    if [ "$day_of_week" -eq 0 ]; then
        ...
        if [ -z "$week_to_delete" ]; then
            ...
            [ -n "$weekly$week_to_delete" ] && rm -rf "$weekly$week_to_delete"
            for week in $week_to_delete; do
                [ -n "$weekly$week" ] && rm -rf "$weekly$week"
            done
            directory_week_count=$(find "$weekly" -mindepth 1 -type d | wc -l)
            directory_daily_count=$(find "$backup_dir" -mindepth 1 -type d | wc -l)
            remote_week_size=$(du -sh "$weekly" | awk -F" " '{print $1}')
        ...
        echo "<tr><td><b>Duration</b></td> <td align=center>$totalTimeTake</td><td><b>Database Growth</b></td> <td align=center>$growth</td></tr>" >>"$bckreport"
        echo "</table> </body> </html>" >>"$bckreport"
        /usr/sbin/sendmail -t <"$bckreport"
        echo "$(date) Done backup of databases " >>"$logfile"
        umount "$mount_point"
        [ -n "$bckreport" ] && rm "$bckreport"
        find /backup/DB_Backup/ -type d -mtime +1 -exec rm -rf {} \;
		echo "{
  ...
  \"Remote_DB_Total_Size\": \"$week_size_json\"
        ...
        if [ -z "$daily_to_delete" ]; then
            ...
            [ -n "$backup_dir$daily_to_delete" ] && rm -rf "$backup_dir$daily_to_delete"
            for daily in $daily_to_delete; do
                [ -n "$backup_dir$daily" ] && rm -rf "$backup_dir$daily"
            done
            directory_week_count=$(find "$weekly" -mindepth 1 -type d | wc -l)
            directory_daily_count=$(find "$backup_dir" -mindepth 1 -type d | wc -l)
            remote_week_size=$(du -sh "$weekly" | awk -F" " '{print $1}')
...
<td align='center'>$(date +"%T")</td>
    ...
    <tr align='center'><td colspan='4'><b>$dbtype Backup Details</b></td></tr>" >"$bckreport"
